...
type Repository interface {
	...
	AttachGoogleID(ctx context.Context, userID, googleID string) error
...
func (r *repository) FindByEmail(ctx context.Context, email string) (*User, error) {
	query := `SELECT id, tenant_id, email, password_hash, password_set_at, google_id, role, created_at, updated_at FROM users WHERE email = $1`

	var user User
	err := r.db.QueryRow(ctx, query, email).Scan(
		&user.ID, &user.TenantID, &user.Email, &user.PasswordHash, &user.PasswordSetAt,
...
func (r *repository) CreateTenantAndUser(ctx context.Context, params CreateUserParams) (*User, error) {
	...
	var user User
	userQuery := `
		INSERT INTO users (tenant_id, email, password_hash, password_set_at, google_id, role) 
		VALUES ($1, $2, $3, $4, $5, $6) 
		RETURNING id, tenant_id, email, password_hash, password_set_at, google_id, role, created_at, updated_at`
	...
	err = tx.QueryRow(ctx, userQuery, tenantID, params.Email, params.PasswordHash, passwordSetAt, params.GoogleID, "admin").Scan(
		&user.ID, &user.TenantID, &user.Email, &user.PasswordHash, &user.PasswordSetAt, &user.GoogleID, &user.Role, &user.CreatedAt, &user.UpdatedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (r *repository) AttachGoogleID(ctx context.Context, userID, googleID string) error {
	_, err := r.db.Exec(ctx, `UPDATE users SET google_id = $2, updated_at = now() WHERE id = $1::uuid`, userID, googleID)
	return err
}
...
func (r *repository) ConsumePasswordResetToken(ctx context.Context, tokenHash, passwordHash string) (*User, error) {
	...
	query := `
		SELECT u.id, u.tenant_id, u.email, u.password_hash, u.password_set_at, u.google_id, u.role, u.created_at, u.updated_at
